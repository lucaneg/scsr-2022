...
public class CharacterInclusionTest {

    @Test
    public void testSigns() throws ParsingException, AnalysisException {
        Program program = IMPFrontend.processFile("inputs/characterInclusion.imp");

        LiSAConfiguration conf = new LiSAConfiguration();
        conf.setJsonOutput(true);
        conf.setDumpAnalysis(true);
        conf.setWorkdir("outputs/characterInclusion");
        conf.setAbstractState(
                new SimpleAbstractState<>(
                        new MonolithicHeap(), // THIS IS THE HEAP DOMAIN
                        new ValueEnvironment<>(new CharacterInclusion()), // THIS IS THE VALUE DOMAIN
